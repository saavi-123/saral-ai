...
export default function ChatThreadPage({ params }) {
  ...
  const fetchAll = async () => {
    const [threadRes, messagesRes, charsRes, contextRes] = await Promise.all([
      // Thread — still uses NEXT_PUBLIC_STRAPI_URL since there's no ownership risk
      // on a single thread fetch (page is already protected by project ownership in middleware)
      // but we route through internal API for consistency
      fetch(`/api/chat-threads-get?threadId=${threadId}`),
      fetch(`/api/messages?threadId=${threadId}`),
      fetch(`/api/characters?projectId=${id}`),
      fetch(`/api/context-items?projectId=${id}`),
    ]);
  ...
  const handleBranch = async (messageIndex) => {
    ...
    setBranchingId(null);
    if (data.newThreadId)
      router.push(`/saral-ai/projects/${id}/chat/${data.newThreadId}`);
  };
  ...
  const productionChars = characters.filter(
    (c) => c.category === "production" || !c.category
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 64px)" }}>
      ...
      >
        <div>
          <Link
            href={`/saral-ai/projects/${id}/chat`}
            ...
            {contextItems.map((item) => (
              <div
                ...
                style={{
                  ...
                  background: selectedContext.includes(item.id)
                    ? "var(--accent)"
                    : "var(--bg3)",
                  color: selectedContext.includes(item.id)
                    ? "var(--accent-text)"
                    : "var(--text2)",
                  border: selectedContext.includes(item.id)
                    ? "none"
        ...
        <div style={{ display: "flex", gap: "10px", alignItems: "flex-start" }}>
            ...
            <textarea
              ...
              placeholder={
                selectedCharacters.length === 0
                  ...
                  : `Simulate scene with ${selectedCharacters
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            <button
              ...
              style={{
                background:
                  selectedContext.length > 0 ? "var(--accent)" : "transparent",
                color:
                  selectedContext.length > 0 ? "var(--accent-text)" : "var(--text2)",
                border: "0.5px solid var(--border2)",
                borderRadius: "8px",
                padding: "8px 14px",
                fontSize: "12px",
            ...
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                opacity:
                  !input.trim() || selectedCharacters.length === 0 || loading ? 0.5 : 1,
                whiteSpace: "nowrap",
              }}
            >
              {loading ? "..." : "Simulate →"}
        ...
        <div style={{ fontSize: "11px", color: "var(--text3)", marginTop: "6px" }}>
          {selectedCharacters.length === 0
            ...
            : `${selectedCharacters.length} character${
              ...
              } selected · Enter to simulate · Hover any message to branch`}
        </div>
      </div>
    </div>
  );
